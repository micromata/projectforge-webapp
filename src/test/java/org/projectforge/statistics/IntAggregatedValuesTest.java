...
public class IntAggregatedValuesTest
{
  ...
  private void assertValues(final int expectedAverage, final int expectedNumberOfElement, final int... values)
  {
    ...
    Assert.assertEquals(expectedAverage, (int) stat.getAverage());
    Assert.assertEquals(expectedNumberOfElement, stat.getNumberOfValues());
  }

  private void assertWeightetValues(final int expectedAverage, final int exptectedWeightedAverage, final int expectedNumberOfElement,
      final int... values)
  {
    final IntAggregatedValues stat = new IntAggregatedValues();
    if (values != null) {
    ...
    Assert.assertEquals(expectedAverage, (int) stat.getAverage());
    Assert.assertEquals(exptectedWeightedAverage, (int) stat.getWeightedAverage());
    Assert.assertEquals(expectedNumberOfElement, stat.getNumberOfValues());
  }
}

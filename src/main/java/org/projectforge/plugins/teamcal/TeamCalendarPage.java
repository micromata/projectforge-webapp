...
public class TeamCalendarPage extends AbstractSecuredPage implements ISelectCallerPage
{
  ...
  protected TeamCalendarPanel calendarPanel;

  protected final PageParameters pageParameters;

  public TeamCalendarPage(final PageParameters parameters)
  {
    super(parameters);
  ...
  public void init()
  {
    form = new TeamCalendarForm(this);
    body.add(form);
    TeamCalendarFilter filter = (TeamCalendarFilter) getUserPrefEntry(USERPREF_KEY);
    if (filter == null) {
      filter = new TeamCalendarFilter();
      putUserPrefEntry(USERPREF_KEY, filter, true);
    }
    form.setFilter(filter);
    form.init();
    calendarPanel = new TeamCalendarPanel("cal", form.currentDatePanel);
   ...
   */
  public TeamCalendarPage setStartDate(final Date startDate)
  {

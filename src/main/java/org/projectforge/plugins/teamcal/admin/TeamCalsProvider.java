...
public class TeamCalsProvider extends TextChoiceProvider<TeamCalDO>
{
  ...
  public List<String> getCalendarNames(final String calIds)
  {
    ...
    for (final int id : ids) {
      final TeamCalDO cal = getTeamCalCache().getCalendar(id);
      if (cal != null) {
  ...
  {
    if (sortedCals == null) {
      final Collection<TeamCalDO> allCalendars = getTeamCalCache().getAllAccessibleCalendars();
      sortedCals = new TreeSet<TeamCalDO>(calsComparator);
  ...
  public Collection<TeamCalDO> toChoices(final Collection<String> ids)
  {
    ...
    for (final String str : ids) {
      ...
      final TeamCalDO cal = getTeamCalCache().getCalendar(calId);
      if (cal != null) {
        list.add(cal);
      }
    }
  ...
  {
    if (teamCalCache == null) {
      // Mustn't be synchronized, nothings happens if assigned multiple times.
      teamCalCache = TeamCalCache.getInstance();
    }
    return teamCalCache;
  }
}

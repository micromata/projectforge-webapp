...
public class SkillTreePage extends AbstractSecuredPage
{
  ...
  private void init()
  {
    ...
    body.add(form);
    form.init();
    skillTreeBuilder = new SkillTreeBuilder().setCaller(caller).setSelectProperty(selectProperty).setSelectMode(isSelectMode()).setShowRootNode(accessChecker.isLoggedInUserMemberOfAdminGroup());

...
public class LicenseListPage extends AbstractListPage<LicenseListForm, LicenseDao, LicenseDO> implements IListPageColumnsCreator<LicenseDO>
{
  private static final long serialVersionUID = -1802352700020870211L;

  @SpringBean(name = "licenseDao")
  ...
  public List<IColumn<LicenseDO, String>> createColumns(final WebPage returnToPage, final boolean sortable)
  {
    ...
    columns.add(new CellItemListenerPropertyColumn<LicenseDO>(new Model<String>(getString("organization")), getSortable("organization", sortable),
      ...
      {
        final LicenseDO license = rowModel.getObject();
        item.add(new ListSelectActionPanel(componentId, rowModel, LicenseEditPage.class, license.getId(), returnToPage, rowModel.getObject().getOrganization()));
        addRowClick(item);
        cellItemListener.populateItem(item, componentId, rowModel);
      }
    });
    columns.add(new CellItemListenerPropertyColumn<LicenseDO>(new Model<String>(getString("plugins.licensemanagement.product")), getSortable(
        "product", sortable), "product", cellItemListener));
    columns.add(new CellItemListenerPropertyColumn<LicenseDO>(new Model<String>(getString("plugins.licensemanagement.version")), getSortable(
    ...
    columns.add(new CellItemListenerPropertyColumn<LicenseDO>(new Model<String>(getString("comment")),
      ...
      {
        final LicenseDO license = rowModel.getObject();
        final Label label = new Label(componentId, new Model<String>(StringUtils.abbreviate(license.getComment(), 100)));
        cellItemListener.populateItem(item, componentId, rowModel);
        item.add(label);
      }
    });

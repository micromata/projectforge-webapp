...
public class HRListPage extends AbstractListPage<HRListForm, HRDao, HRViewUserData> implements ISelectCallerPage
{
  ...
  private void addListEntry(final Item<ICellPopulator<HRViewUserData>> item, final String componentId, final BigDecimal plannedDays,
      final BigDecimal actualDays, final Link< ? > link)
  {
    final HRListEntryPanel entryPanel = new HRListEntryPanel(componentId, form.getSearchFilter(), plannedDays, actualDays, link);
    item.add(entryPanel);
  }
  ...
  {
    final DivPanel panel = new DivPanel(id, DivType.GRID16, DivType.BOX, DivType.ROUND_ALL);
    form.add(panel);

...
public abstract class UserRight implements Serializable
{
  ...
  public UserRight(final UserRightId id, final UserRightCategory category)
  {
    this.id = id;
    this.category = category;
  }

  public UserRight(final UserRightId id, final UserRightCategory category, final UserRightValue... values)
  {
    this(id, category);
    this.values = values;
  }

  public UserRight(final UserRightId id, final UserRightCategory category, final UserRight dependsOn)
  {
   ...
   */
  public UserRight(final UserRightId id, final UserRightCategory category, final UserRight dependsOn, final UserRightValue... values)
  {
    this(id, category);
  ...
  /**
   * For some users and right combinations it's possible, that the user has an access value at default because he's member of a group
   * with a single right. <br/>
   * @param userGroupCache
   * @param user
   * @return true if all available values for the user matches automatically, otherwise false (this right seems to be configurable for the
   *         given user in the UserEditForm).
   */
  ...
  /**
   * Get all right values which are potentially available for the user. If the user value stored in the user data is not part of available
   * values then the AccessChecker returns that the user has no right.
   * @param userGroupCache
   * @param user
   * @return
   */
  ...
  /**
   * @param userGroupCache
   * @return isAvailable(userGroupCache, user) at default or if dependsOn is given dependsOn.available(userGroupCache, user, value);
   */
  ...
  /**
   * If a right should match independent on the value set for the given user. Should only be called, if the UserRightDO or its value is set
   * to null.
   * @param userGroupCache
   * @param user
   * @param value
  ...
  {
    return values != null && values.length == 2 && values[0] == UserRightValue.FALSE && values[1] == UserRightValue.TRUE;
  }

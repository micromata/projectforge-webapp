	...
	function requestData(q) {
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			$.ajax({
				...
				type: "POST",
				data: {_eventName:options.event, q : q}, 
...
jQuery.fn.autocomplete = function(url, event, options, data) {
	// Make sure options exists
	options = options || {};
	// Set url as option
	options.url = url;
	// Set the Stripes action method as option
	options.event = event;
	...
	options.favoriteEntries = options.favoriteEntries || 0; // List of top entries to show on double click
	options.scroll = options.scroll || true; // Show scrollbars if result list is larger than scrollHeight?
	options.scrollHeight = options.scrollHeight || 400; // Height of result area in pixel

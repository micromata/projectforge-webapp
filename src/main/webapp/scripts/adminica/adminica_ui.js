...
$(function() {
	...
	$("#nav_top > ul > li").each(
			function() {
				$(this).children("ul").addClass("dropdown").parent().addClass(
						"has_dropdown");
			});
	...
	$(".has_drawer > a").click(
			function() {
				var menuType = ($(this).parent().parent()
						.hasClass("open_multiple"));
				if (menuType != true) {
					$(this).parent().siblings().removeClass("open").children(
							"ul.drawer").slideUp();
				}
				$(this).parent().toggleClass("open").children("ul")
						.slideToggle();
				return false;
			});
	...
	// Loading Box - tweak the delays to your liking // Kai: uncommented.
	...
	$("a.toggle")
			.click(
					function() {
						$(this).toggleClass("toggle_closed").next()
								.slideToggle("fast");
						$(this).siblings(".box_head").removeClass("round_top")
								.toggleClass("round_all");
						return false; // Prevent the browser jump to the link
										// anchor
					});
	...
	$(".content_accordion").accordion({
		collapsible : true,
		active : false,
		header : 'h3.bar', // this is the element that will be clicked to
